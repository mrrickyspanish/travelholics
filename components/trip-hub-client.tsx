...
type HubMode = 'live' | 'preview' | 'archived'
...
export default function TripHubClient({ trip, cabins, itinerary, deadlines, isLeader, initialParties, mode }: { trip: Trip; cabins: Cabin[]; itinerary: Itinerary[]; deadlines: Deadline[]; isLeader: boolean; initialParties: Party[]; mode: HubMode }) {
  const interactive = mode === 'live'
  const [selectedCabin, setSelectedCabin] = useState('')
  const [bookingState, setBookingState] = useState<'idle' | 'sending' | 'success'>('idle')
  const [bookingError, setBookingError] = useState('')
  const [bookingResult, setBookingResult] = useState<{ alreadySubmitted: boolean; message: string } | null>(null)
  ...
  async function submitBooking(event: React.FormEvent<HTMLFormElement>) {
    ...
    setBookingResult({
      ...
      message: data.message || 'We sent you a confirmation email so there is no wondering what happens next. Yolanda will review your request and follow up.',
    })
  ...
  return (
    <main className="bg-sand text-ink">
      {mode !== 'live' ? <div className={`px-5 py-3 text-center text-sm font-bold sm:px-8 ${mode === 'preview' ? 'bg-amber-100 text-amber-900' : 'bg-slate-200 text-slate-800'}`}>{mode === 'preview' ? 'Leader preview · This Trip Hub is still a draft. Guests cannot enter yet, and invites/booking requests are paused until Yolanda publishes it.' : 'Archived · Read-only reference mode. Existing authorized travelers and the group leader can still view trip details, but new activity is paused.'}</div> : null}
      ...
      {isLeader ? (
        <section className="border-b border-ink/8 bg-cream px-5 py-8 sm:px-8 lg:px-12">
          <div className="mx-auto max-w-7xl">
            <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.15em] text-coral">Group Leader View</p><h2 className="mt-1 font-serif text-3xl font-semibold text-royal-deep">Your crew at a glance</h2></div><p className="max-w-md text-sm text-stone">{interactive ? 'Names and booking progress only. You can correct or remove invitations until a guest responds. Travelholics handles the booking work.' : 'This is a read-only leader view while the Trip Hub is not live. You can review the group progress without changing invitations.'}</p></div>
      ...
      <section id="booking-request" className="scroll-mt-6 px-5 py-14 sm:px-8 sm:py-18 lg:px-12 lg:py-20">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.34fr_0.66fr] lg:items-start"><div><p className="text-xs font-black uppercase tracking-[0.15em] text-coral">{interactive ? 'Ready to join the cruise?' : 'Trip Hub reference'}</p><h2 className="mt-2 font-serif text-4xl font-semibold tracking-[-0.04em] text-royal-deep">{interactive ? 'Send Yolanda your booking request.' : mode === 'preview' ? 'Booking requests open when the trip is published.' : 'New booking requests are closed.'}</h2><p className="mt-4 text-sm leading-6 text-stone">{interactive ? (trip.bookingRequestNote || 'Choose your cabin preference and tell us who is traveling. Yolanda will follow up to confirm availability and complete the booking with you.') : 'You can still review the trip details above. Contact Travelholics directly if you need help with an existing booking.'}</p><div className="mt-5 rounded-xl border border-ink/8 bg-cream p-4 text-xs font-semibold leading-5 text-stone">{interactive ? 'No payment is collected here. Your submission tells Travelholics what you want so Yolanda can take the next step with you.' : 'No new information can be submitted from this read-only view.'}</div></div>
          {!interactive ? <div className="rounded-[2rem] border border-ink/8 bg-cream p-8 text-center shadow-sm"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-mid text-white"><CheckCircle2 size={23} /></span><h3 className="mt-5 font-serif text-3xl font-semibold text-royal-deep">Trip details are available for reference.</h3><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-stone">Questions or changes? Contact Yolanda at trips@yotravelholic.com.</p></div> : bookingState === 'success' ? <div className="rounded-[2rem] border border-emerald-mid/20 bg-cream p-8 text-center shadow-sm"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-mid text-white"><CheckCircle2 size={23} /></span><h3 className="mt-5 font-serif text-3xl font-semibold text-royal-deep">{bookingResult?.alreadySubmitted ? 'Already with Travelholics.' : 'Request received.'}</h3><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-stone">{bookingResult?.message}</p>{bookingResult?.alreadySubmitted ? <p className="mx-auto mt-3 max-w-xl text-sm font-semibold leading-6 text-emerald-deep">Your original request was not overwritten. To correct traveler, cabin, phone, or party details, contact Yolanda at trips@yotravelholic.com.</p> : null}</div> : <form onSubmit={submitBooking} className="rounded-[2rem] border border-ink/8 bg-cream p-5 shadow-sm sm:p-7"><div className="grid gap-5 sm:grid-cols-2"><label className="text-base font-bold text-ink">Primary contact *<input required name="primaryName" className="mt-1.5 w-full rounded-xl border border-ink/10 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-mid" /></label><label className="text-base font-bold text-ink">Email *<input required type="email" name="email" className="mt-1.5 w-full rounded-xl border border-ink/10 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-mid" /></label><label className="text-base font-bold text-ink">Phone *<input required name="phone" className="mt-1.5 w-full rounded-xl border border-ink/10 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-mid" /></label><label className="text-base font-bold text-ink">Number of travelers *<input required type="number" min="1" name="partySize" className="mt-1.5 w-full rounded-xl border border-ink/10 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-mid" /></label><label className="text-base font-bold text-ink sm:col-span-2">Cabin preference<select name="cabinOfferId" value={selectedCabin} onChange={(e) => setSelectedCabin(e.target.value)} className="mt-1.5 w-full rounded-xl border border-ink/10 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-mid"><option value="">Choose a cabin</option>{cabins.map((cabin) => <option key={cabin.id} value={cabin.id}>{cabin.name}</option>)}</select><input type="hidden" name="cabinPreference" value={cabins.find((cabin) => cabin.id === selectedCabin)?.name || ''} /></label><label className="text-base font-bold text-ink sm:col-span-2">Who else is traveling in your cabin?<textarea name="members" className="mt-1.5 min-h-24 w-full rounded-xl border border-ink/10 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-mid" placeholder={'One traveler name per line\nAshley Barnes\nGreyson Barnes'} /></label><label className="text-base font-bold text-ink sm:col-span-2">Anything Yolanda should know?<textarea name="notes" className="mt-1.5 min-h-20 w-full rounded-xl border border-ink/10 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-mid" /></label></div>{bookingError ? <p className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{bookingError}</p> : null}<button disabled={bookingState === 'sending'} className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-coral px-5 py-3 text-base font-bold text-white hover:bg-coral-deep disabled:opacity-50"><Plus size={18} /> {bookingState === 'sending' ? 'Sending request…' : 'Send My Booking Request'}</button></form>}
        </div>
      </section>
    </main>
  )
