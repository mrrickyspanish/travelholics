...
export default function TripAccessGate({ slug, tripName, cruiseLine, ship }: { slug: string; tripName: string; cruiseLine: string; ship: string }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setLoading(false)
    if (!response.ok) {
      if (response.status === 429 && Number(data.retryAfter) > 0) setCooldown(Number(data.retryAfter))
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-emerald-deep px-5 py-24 text-white">
      ...
      <form onSubmit={submit} className="relative w-full max-w-lg rounded-[2rem] border border-white/15 bg-white/10 p-6 text-center shadow-2xl backdrop-blur-xl sm:p-8">
        ...
        <p className="mx-auto mt-6 max-w-sm text-base leading-6 text-white/70">Enter the shared access code from your Travelholics invitation to open the group trip.</p>
        <label className="mt-6 block text-left text-xs font-bold uppercase tracking-wide text-white/55">Group access code<input autoFocus required disabled={cooldown > 0} value={code} onChange={(e) => setCode(e.target.value)} className="mt-2 w-full rounded-xl border border-white/15 bg-white px-4 py-3.5 text-center font-mono text-lg font-bold tracking-[0.12em] text-ink outline-none focus:border-coral focus:ring-2 focus:ring-coral/20 disabled:cursor-not-allowed disabled:opacity-60" placeholder="YOURCODE" /></label>
        {error ? <p role="alert" className="mt-3 rounded-xl bg-red-500/15 px-3 py-2 text-sm font-semibold text-red-100">{error}</p> : null}
        <button type="submit" disabled={loading || cooldown > 0} className="mt-4 min-h-12 w-full rounded-xl bg-coral px-5 py-3 text-base font-bold text-white transition hover:bg-coral-deep disabled:cursor-not-allowed disabled:opacity-50">{cooldown > 0 ? `Try again in ${cooldownLabel(cooldown)}` : loading ? 'Opening…' : 'Open My Group Trip'}</button>
