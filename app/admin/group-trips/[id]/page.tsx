...
import { ArrowLeft, CalendarDays, KeyRound, MapPin, Ship, UserRound } from 'lucide-react'
    ...
    <div className="min-h-full bg-[#f7f8f5] px-5 py-7 sm:px-8 lg:px-10 lg:py-10">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-7 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="mb-2 flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide ${trip.status === 'published' ? 'bg-emerald-100 text-emerald-800' : trip.status === 'archived' ? 'bg-slate-100 text-slate-600' : 'bg-amber-100 text-amber-800'}`}>{trip.status}</span><span className="text-xs text-[#829089]">/{trip.slug}</span></div>
        ...
        <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-[#e2e8e5] bg-white p-4"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-[#829089]"><CalendarDays size={14} /> Sailing</p><p className="mt-2 text-sm font-semibold text-[#10251e]">{dateLabel(trip.sail_date)}</p><p className="mt-0.5 text-xs text-[#829089]">Return: {dateLabel(trip.return_date)}</p></div>
          <div className="rounded-2xl border border-[#e2e8e5] bg-white p-4"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-[#829089]"><MapPin size={14} /> Departure</p><p className="mt-2 text-sm font-semibold text-[#10251e]">{trip.departure_port || 'Not set'}</p><p className="mt-0.5 text-xs text-[#829089]">{trip.destination}</p></div>
          <div className="rounded-2xl border border-[#e2e8e5] bg-white p-4"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-[#829089]"><Ship size={14} /> Ship</p><p className="mt-2 text-sm font-semibold text-[#10251e]">{trip.ship}</p><p className="mt-0.5 text-xs text-[#829089]">{trip.cruise_line}</p></div>
          <div className="rounded-2xl border border-[#e2e8e5] bg-white p-4"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-[#829089]"><UserRound size={14} /> Group leader</p><p className="mt-2 text-sm font-semibold text-[#10251e]">{trip.group_leader_name}</p><p className="mt-0.5 truncate text-xs text-[#829089]">{trip.group_leader_email}</p></div>
        </div>
